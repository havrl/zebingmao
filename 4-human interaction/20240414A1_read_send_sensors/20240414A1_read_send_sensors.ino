...
void loop() {
 if (Serial.available() > 0) { // 检查是否有来自串行端口的数据
    char command = Serial.read(); // 读取数据
    if (command == 'H') {
      digitalWrite(ledPin, HIGH); // 如果接收到'H'，LED点亮
    } else if (command == 'L') {
      digitalWrite(ledPin, LOW); // 如果接收到'L'，LED熄灭
    }
  }
...
// A5 A12B21 L_A12B21 = 80  R_A12B21 =0.60M
  ...
  float R_A12B21 = (10*voltage5)/(5.0-voltage5)/4.64+0.35; 
...
// A7 A11C11 L_A11C11 = 75  R_A11C11 =1.02M
  ...
  float R_A11C11 = (10*voltage7)/(5.0-voltage7)/4.64+0.64;
